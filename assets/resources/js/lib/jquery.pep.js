...
;(function ( $, window, undefined ) {
    ...
    var self = this;
            ...
            if ( this.isValidMoveEvent(ev) && !this.disabled ){
                    ...
                    (function watchMoveLoop(){
                    	console.log('watchMoveLoop');
                    	
                        if ( !self.active ) return;
                        if(self.lastMoveLoop){
                        	self.lastMoveLoop();
                        	self.lastMoveLoop = null;
                        } else {
	                        self.handleMove();
  ...
  Pep.prototype.handleMove = function() {
            ...
            if ( typeof(this.moveEvent) === 'undefined' || (!this.isTouch(this.moveEvent) && this.options.manageTouchOnlyEvents === true ))
              return;

            // get our move event's x & y
            var ev      = this.normalizeEvent( this.moveEvent );
            console.debug('ev.type:' + ev.type + 'ev.pepx:' + ev.pep.x + ' ev.pepy:' + ev.pep.y);
  ...
  Pep.prototype.handleStop = function(ev) {
            ...
            var self = this;
            this.lastMoveLoop = function(){
            
	            // make object inactive, so watchMoveLoop returns
            	self.active = false;
	
	            // remove our start class
            	self.$el.removeClass('pep-start');
	            ...
	            if ( this.started || (!self.started &&  $.inArray('stop', self.options.callIfNotStarted) > -1 ) ) {
	            	self.options.stop.call(self, ev, self);
	            }
	
	            // ease the object, if necessary.
	            if (self.options.shouldEase)
	            	self.addClass('pep-ease').ease(ev, self.started);
	            // else remove active class
	            else 
	            	self.$el.removeClass( [self.options.activeClass].join(' ') );
